...
contract Staking is IStaking, Context, Ownable , ReentrancyGuard {
    ...
    mapping(address => uint8) private unStake_user_count;
    ...
    function stake(uint256 amount) public nonReentrant{ 
     ...
     function unStake(uint256 amount) public nonReentrant returns(bool){ 
        ...
        if(block.timestamp < detail.stakeTime + 1 weeks){
           ...
           if(unStake_user_count[_msgSender()] == 0){
            uint256 withdraw_rate = 100 - feeRate ;
            uint256 amount_80 = (amount/100) * withdraw_rate;
            fee += amount - amount_80;
            detail.depositValue = detail.depositValue - amount_80;
            savePendingRewards();
            pool_Dec(balance);
            pool_Inc(detail.depositValue);
            freeze(_msgSender());
            detail.tickets = getTicketsForUser(detail.depositValue);
            detail.userWeight = getPoolWeight(detail.depositValue);
            userStakingDetail[_msgSender()] = detail;
            totalStakedValue -= amount_80;
            return true;
           }
    ...
    function resetFreeze(address account) private {
         stakingDetail memory detail = userStakingDetail[account];
         if(block.timestamp > detail.stakeTime + 1 weeks){
           unStake_user_count[account] = 0;
        }
    }

    function freeze(address account) private {
         stakingDetail memory detail = userStakingDetail[account];
         if(block.timestamp < detail.stakeTime + 1 weeks){
         unStake_user_count[account] = 1;
        }
    }
}